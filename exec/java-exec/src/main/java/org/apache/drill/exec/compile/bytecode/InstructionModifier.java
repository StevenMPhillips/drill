...
public class InstructionModifier extends MethodVisitor {
   ...
   * size must be increased by one to accomodate the extra DUP that is used to do so.
   */
  ...
  public void visitInsn(int opcode) {
    ...
    case Opcodes.DUP2_X1: {
      ...
      if (AsmUtil.isXconst(nextInsn.getOpcode())) {
        final AbstractInsnNode nextInsn2 = nextInsn.getNext();
        if (AsmUtil.isXadd(nextInsn2.getOpcode())) {
  ...
  public void visitVarInsn(final int opcode, final int var) {
    ...
    if (opcode == Opcodes.ASTORE && (v = peekFromTop(0)) != null) {
      if (!v.isFunctionReturn()) {
        ...
        // TODO(cwestin): should we also do this for "out" parameters? if so, we'll have to move values
        // them into them before functions are called. might be easier (and fewer instructions) to not do that
        int first;
        ...
        // both branches above transfer to locals
        transferredToLocals = true;
    ...
    } else if (opcode == Opcodes.ALOAD && (v = getFunctionReturn()) != null) {
  ...
  public void visitFieldInsn(final int opcode, final String owner,
    ...
    case Opcodes.GETFIELD:
      ...
      if (value != null) {
        if (!value.isFunctionReturn()) {
          ValueHolderSub sub = oldToNew.get(value.getIndex());
