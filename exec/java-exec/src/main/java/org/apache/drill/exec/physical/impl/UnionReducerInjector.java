...
public class UnionReducerInjector extends AbstractPhysicalVisitor<PhysicalOperator, FragmentContext, ExecutionSetupException> {

  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(UnionReducerInjector.class);
  ...
  public static FragmentRoot rewritePlanWithIteratorValidator(FragmentContext context, FragmentRoot root) throws ExecutionSetupException {
    UnionReducerInjector inject = new UnionReducerInjector();
  ...
  /* Get the list of child operators */
    for (PhysicalOperator child : op) {
      if (child.getSVMode() != SelectionVectorMode.NONE) {
        newChildren.add(child.accept(this, context));
        continue;
      }
      PhysicalOperator reducer = new UnionTypeReducer(child.accept(this, context));
      reducer.setOperatorId(op.getOperatorId() + 1000);
      newChildren.add(reducer);
    }
