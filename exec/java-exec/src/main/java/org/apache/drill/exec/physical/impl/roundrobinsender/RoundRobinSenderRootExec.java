 ...
 */
public class RoundRobinSenderRootExec extends BaseRootExec {
  ...
  public RoundRobinSenderRootExec(FragmentContext context,
                                  ...
                                  RoundRobinSender config) throws OutOfMemoryException {
  ...
  public boolean innerNext() {
    ...
    switch(out){
      ...
      case OK:
        ...
        int receiver = batchCount++ % tunnels.length;
        FragmentWritableBatch batch = new FragmentWritableBatch(false, handle.getQueryId(), handle.getMajorFragmentId(), handle.getMinorFragmentId(), config.getOppositeMajorFragmentId(), receiver, writableBatch);
        updateStats(batch);
        stats.startWait();
        try {
          tunnels[receiver].sendRecordBatch(this.statusHandler, batch);
        } finally {
          stats.stopWait();
        }
        statusHandler.sendCount.increment();
...
*/
  ...
  private class StatusHandler extends BaseRpcOutcomeListener<Ack> {
